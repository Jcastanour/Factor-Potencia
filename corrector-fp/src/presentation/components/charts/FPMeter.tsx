...
type Props = { fp: number; size?: number; className?: string };
...
export function FPMeter({ fp, size = 220, className }: Props) {
  ...
  const color = QUALITY_COLOR[fpQuality(clamped)];
  ...
  return (
    ...
    >
      ...
      />
      <line x1={cx} y1={cy} x2={nx} y2={ny} stroke="var(--fg-primary)" strokeWidth="2" strokeLinecap="round" />
      ...
      {[0.88, 0.97].map((t) => {
        ...
        return (
          <line key={t} x1={tx1} y1={ty1} x2={tx2} y2={ty2} stroke="var(--fg-dim)" strokeWidth="1.5" />
        );
      })}
